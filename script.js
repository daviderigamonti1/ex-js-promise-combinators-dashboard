...
async function getDashboardData(query) {
    try {
        const destinationPromise = await fetchJson(`${apiUrl}/destinations?search=${query}`);
        const weatherPromise = await fetchJson(`${apiUrl}/weathers?search=${query}`);
        const airportPromise = await fetchJson(`${apiUrl}/airports?search=${query}`);
